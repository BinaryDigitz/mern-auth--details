...
export const signUp = asyncMiddleware(async (req, res) => {
    ...
    return res.json(
      authSuccess("User created successfully", 201, { userToken: token, user: rest })
    );
});
...
export const signIn = asyncMiddleware(async (req, res) => {
 ...
 if(!user) return res.json(authFail('User not found', 404))

    const isPasswordValid = await bcrypt.compare(password, user.password)
    if(!isPasswordValid) return res.json(authFail('Invalid password'))
    ...
    const { password: pass, ...rest } = user._doc
    return res.json(authSuccess('Fetch successfully', 200, { user: rest, userToken: token}))
});
